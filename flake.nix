    ...
    utils.apply-systems
      ...
      (
        ...
        rec {
          legacyPackages =
            let
              purescripts = listToAttrs (
                map
                  (
                    file:
                    let
                      package-name = l.pipe file [
                        (l.removeSuffix ".nix")
                        (replaceStrings [ "." ] [ "_" ])
                        (a: "purescript-${a}")
                      ];
                    in
                    l.nameValuePair package-name (
              ...
              // purescripts;
            in
            { inherit for-0_15 for-0_14 for-0_13; } // packages;

          checks =
            let
              packages-for-version =
                version:
                let
                  name = "packages for version ${version}";
                in
                {
                  ${name} =
                    p.runCommand name
            ...
            in
            {
              ...
              "everything builds" = p.runCommand "build-everything" { } (
          ...
          devShells.default = make-shell {
            ...
            ++ [ legacyPackages.purescript-language-server ];
